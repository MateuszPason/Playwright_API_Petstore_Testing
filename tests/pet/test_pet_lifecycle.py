...
def test_petet_lifecycle(pet: Pet, pet_cleanup):
    ...
    response_create_pet = pet.create_pet(
        "/v2/pet",
    ...
    response_update_pet = pet.update_pet(
        "/v2/pet",
        data=UPDATE_PET.__dict__
    )
    ...
    response_delete_pet = pet.delete_pet(f"/v2/pet/{pet_id}")
    assert response_delete_pet.ok

    # Get pet with pet_id to check if it doesn't exist

    tested_pet = pet.get_pet(f'/v2/pet/{pet_id}')
