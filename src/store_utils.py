...
class Store:
    ...
    def get_order_by_id(self, endpoint, order_id, **kwargs):
        if not isinstance(order_id, int) or order_id <= 0:
            raise ValueError("Order ID should be greater than zero")
        return self._request.get(f"{endpoint}/{order_id}", **kwargs)

    def place_an_order(self, order_id, pet_id, quantity, ship_date, status, complete, **kwargs):
        ...
        if not isinstance(status, str) or status not in ["placed", "approved", "delivered"]:
            raise ValueError("Incorrect status | Expected string value of 'placed', 'approved' or 'delivered'")
